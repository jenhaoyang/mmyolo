...
from ..layers import (CSPLayerWithTwoConv, DarknetBottleneck, SPPCSPBlock,
                      SPPFBottleneck, Yolov4CSPLayer, Yolov4CSPTinyLayer)
from ..utils import make_divisible, make_round
from .base_backbone import BaseBackbone


@MODELS.register_module()
class YOLOv4CSPDarknet(BaseBackbone):
    ...
    Example:
        >>> from mmyolo.models import YOLOv4CSPDarknet
        >>> import torch
        >>> model = YOLOv4CSPDarknet()
        >>> model.eval()
    ...
    def build_stem_layer(self) -> nn.Module:
        ...
        kernel_size = 3
        # auto-pad,
        padding = kernel_size // 2 if isinstance(
            kernel_size, int) else [x // 2 for x in kernel_size]
        return ConvModule(
            self.input_channels,
...
@MODELS.register_module()
class YOLOv4TinyBackbone(BaseBackbone):
    """CSP-Darknet backbone used in YOLOv5.
    Args:
        arch (str): Architecture of CSP-Darknet, from {P5, P6}.
            Defaults to P5.
    ...
    # in_channels, out_channels, num_blocks, add_identity, use_spp
    arch_settings = {
        'Tiny': [[64, 128, True, False], [128, 256, True, False],
                 [256, 512, True, False]]
    }
    ...
    def build_stage_layer(self, stage_idx: int, setting: list) -> list:
        ...
        Args:
            ...
            setting (list): The architecture setting of a stage layer.
        """
        in_channels, out_channels, add_identity, use_spp = setting

        stage = []
        tiny_layer = Yolov4CSPTinyLayer(
            in_channels,
            out_channels,
            norm_cfg=self.norm_cfg,
            act_cfg=self.act_cfg)
        stage.append(tiny_layer)
        return stage

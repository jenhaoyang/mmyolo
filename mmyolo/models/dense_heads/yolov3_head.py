...
class YOLOv3HeadModule(BaseModule):
    ...
    def __init__(self,
        ...
        if isinstance(in_channels, int):
            self.in_channels = [in_channels] * self.num_levels
        else:
            self.in_channels = [i for i in in_channels]
    ...
    def _init_layers(self):
        ...
        for i in range(self.num_levels):
            conv_pred = nn.Sequential(
                ...
                nn.Conv2d(2 * self.in_channels[i],
                          self.num_base_priors * self.num_out_attrib, 1))

            self.convs_pred.append(conv_pred)

    def init_weights(self):
        """Initialize the bias of YOLOv3 head."""
        super().init_weights()
...
class YOLOv3Head(BaseDenseHead):
    ...
    def __init__(self,
                 ...
                 obj_level_weights: List[float] = [4.0, 1.0],
